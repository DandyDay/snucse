...
object Assignment3A:
  ...
  def decSqrt(n: LazyList[Int], len: Int): LazyList[Int] = {
    def findX(p: Int, rem: Int): Int = {
      @tailrec
      def findXHelper(x: Int): Int = {
        if (x * (20 * p + x) <= rem) x
        else findXHelper(x - 1)
      }
      findXHelper(9) // x는 최대 9부터 시작
    }

    def updateRem(lst: LazyList[Int], rem: Int): (Int, LazyList[Int]) = {
      lst.get match {
        case Some((hd, tl)) => (rem * 10 + hd, tl)
        case None => (rem * 10, new LNil[Int]())
      }
    }

    def calculateSqrt(rem: Int, p: Int, digitsLeft: Int, lst: LazyList[Int]): LazyList[Int] = {
      if (digitsLeft == 0) new LNil[Int]()
      else {
        if (digitsLeft == len + 1) {  // integer part
          val (newRem, newList) = updateRem(lst, rem)
          val x = findX(p, newRem)
          val updatedRem = newRem - x * (20 * p + x)
          val updatedP = 10 * p + x
          new LCons(x, calculateSqrt(updatedRem, updatedP, digitsLeft - 1, newList))
        }
        else {  // fractional part
          val (tempRem, tempList) = updateRem(lst, rem)
          val (newRem, newList) = updateRem(tempList, tempRem)
          val x = findX(p, newRem)
          val updatedRem = newRem - x * (20 * p + x)
          val updatedP = 10 * p + x
          new LCons(x, calculateSqrt(updatedRem, updatedP, digitsLeft - 1, newList))
