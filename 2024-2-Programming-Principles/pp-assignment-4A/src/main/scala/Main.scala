...
/**
  ...
  */
  ...
  implicit def lazylistListIF[A]: ListIF[Lazylist,A] = new {
    def empty[A] = LNil()
    def cons[A] (a: A, l: =>Lazylist[A]) = LCons(a, l)
    def iter[A] = lazylistIter[A]
    def head[A] (l: Lazylist[A]) = l.get match
      case None => None
...
/**
  ...
  */
  def fibo[L[_]] (implicit listIF : ListIF[L,Long]) : L[Long] = {
    def makeFibo(a: Long, b: Long) : L[Long] = listIF.cons(a, makeFibo(b, a+b))
    makeFibo(0, 1)
  }
...
/**
  ...
  */
  def getN[A, L[_]](l: L[A], n: Int) (implicit listIF : ListIF[L,A]) : L[A] = {
    if (n == 0) listIF.empty
    else listIF.iter.get(l) match
      case None => listIF.empty
      case Some(hd, tl) => listIF.cons(hd, getN(tl, n-1))
  }
...
/**
  ...
  */
  def toList[A, L[_]](x: L[A]) (implicit listIF : ListIF[L,A]) : List[A] = {
    listIF.iter.get(x) match
      case None => Nil
      case Some(hd, tl) => hd :: toList(tl)
  }
